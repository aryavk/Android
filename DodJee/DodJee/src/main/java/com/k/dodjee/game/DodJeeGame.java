...
import com.k.dodjee.graphics.Power;
import com.k.dodjee.graphics.PowerUp;
import com.k.dodjee.util.ActionHandler;
import com.k.dodjee.util.CollisionDetector;
import com.k.dodjee.graphics.Opponent;
import com.k.dodjee.graphics.Person;

import java.math.BigDecimal;
import java.util.ArrayList;

public class DodJeeGame extends GamePanelImpl
{
    ...
    public DodJeeGame(Context context)
    {
        super(context);
    ...
    public DodJeeGame(Context context, GamePanelResources gamePanelResources)
    {
        super(context);
    ...
    public void initialise()
    {
        ...
        if (!isInitialised())
        {
            ...
            getGamePanelResources().getPlayer().getBall().reinitialise();
            getGamePanelResources().getPlayer().getBall().setGameHeight(getGamePanelResources().getGameHeight());
            ...
            for (Opponent opponent : getGamePanelResources().getOpponents())
            {
                ...
                opponent.getBall().setGameHeight(getGamePanelResources().getGameHeight());
            }
    ...
    private void lose()
    {
        ...
        DodJeeActivity activity = ((DodJeeActivity)getContext());
    ...
    private void activatePowerUp(PowerUp powerUp)
    {
        ...
        {
            getGamePanelResources().getPlayer().getBall().setMaxCollissions(2);
    ...
    private void checkBallCollisions(Ball opponentVball)
    {
        // If balls collide then clear both balls
        if (CollisionDetector.collisionDetected(getGamePanelResources().getPlayer().getBall(), opponentVball))
        {
            getGamePanelResources().getPlayer().getBall().setCollided(true);

            if (getGamePanelResources().getPlayer().getBall().isCollided())
            {
                getGamePanelResources().getPlayer().getBall().setMovingVertically(false);
                getGamePanelResources().getPlayer().getBall().reinitialise();
            }
    ...
    private void checkPlayerCollision(Ball opponentVball)
    {
    ...
    private void checkPlayerLiberoCollision(Ball opponentVball)
    {
    ...
    private void checkOpponentCollisions(Opponent opponent)
    {
        ...
        if (CollisionDetector.collisionDetected(getGamePanelResources().getPlayer().getBall(), opponent))
        {
            getGamePanelResources().getPlayer().getBall().setCollided(true);
            ...
            opponent.getBall().setVisible(false);
        }
    }
    ...
    private void checkCollisions()
    {
        ...
        for (Opponent opponent : getGamePanelResources().getOpponents())
        {
            if (opponent.getBall().getY() - (opponent.getBall().getHeight() * 2) < getGamePanelResources().getPlayer().getY())
                checkPlayerCollision(opponent.getBall());

            if (getGamePanelResources().getPlayer().isHasLibero())
            {
                if (opponent.getBall().getY() - (opponent.getBall().getHeight() * 2) < getGamePanelResources().getPlayerLibero().getY())
                    checkPlayerLiberoCollision(opponent.getBall());
            }

            if (getGamePanelResources().getPlayer().getBall().isInPath(opponent.getBall()))
                checkBallCollisions(opponent.getBall());

            if (getGamePanelResources().getPlayer().getBall().isMovingVertically() &&
                    (getGamePanelResources().getPlayer().getBall().getY() + (getGamePanelResources().getPlayer().getHeight() * 2) > opponent.getY()))
    ...
    private void randomiseOpponent(Opponent opponent)
    {
        if (opponent.isVisible())
        {
            if (!opponent.getBall().isMovingVertically())
            {
                opponent.getBall().reinitialise();
                opponent.getBall().setMovingVertically(getGamePanelResources().isGameStarted());
            }

            randomiseOpponentX(opponent);

            if (opponent.getBall().isMovingVertically() && getGamePanelResources().isGameStarted())
            {
                opponent.getBall().moveVertically(this);
                opponent.getBall().spinGraphic();
            }
    ...
    private void incrementSpeed(Person person)
    {
        if (i % 100 == 0)
        {
            if (person.getBall().getSpeed() < maxBallSpeed)
                person.getBall().setSpeed(person.getBall().getSpeed() + 1);
        }
    ...
    public void update(Canvas canvas)
    {
        if (!disableMovement)
        {
            ...
            if (getGamePanelResources().getPlayer().getBall().isMovingVertically() && getGamePanelResources().isGameStarted())
            {
                getGamePanelResources().getPlayer().getBall().moveVertically(this);
                getGamePanelResources().getPlayer().getBall().spinGraphic();
            }
    ...
    public void draw(@NonNull Canvas canvas)
    {
        ...
        if (getGamePanelResources().getPlayer().getBall().isMovingVertically())
        {
            getGamePanelResources().getPlayer().getBall().draw(canvas);
        }
